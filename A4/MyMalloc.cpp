...
void* Allocator::allocate(size_t bytesToAllocate) {
    ...
    MyHashNode* node = static_cast<MyHashNode *> (mmap(nullptr, sizeof(MyHashNode), PROT_READ | PROT_WRITE,
                                                       MAP_PRIVATE | MAP_ANONYMOUS, -1, 0));
    if(node == MAP_FAILED) {
        std::cerr << "mmap failed - allocate MyHashNode" << std::endl;
        munmap(ptr, totalSize); // 清理先前分配的内存块
        exit(1);
    }
    node->pointer_ = ptr;
