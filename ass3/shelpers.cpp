...
#include <iostream>
#include <string>
#include <cstdlib> // For using getenv
...
// Splits a word in the vector at the given index based on a symbol
bool splitOnSymbol(std::vector<std::string>& words, int i, char c) {
    ...
    if ((pos = words[i].find(c)) != std::string::npos) {
        if (pos == 0) {
            ...
            // Symbol is in the middle or at the end
    ...
    for (size_t i = 0; i < tokens.size(); ++i) {
        if (tokens[i] == ">" || tokens[i] == "<") {
            if (i + 1 >= tokens.size()) {
                ...
                return {}; // Return an empty command list on error
            }
            int fd;
            if (tokens[i] == ">") {
...
// Overloading the << operator to print Command objects
